...
def render_admin() -> None:
		...
		if sub:
			...
			try:
				with get_session() as audit_sess:
					audit_sess.add(AuditLog(user_id=current_user_id, action=action, details=f"{emp_id}:{name}:{role}:{zone}/{region}/{city}/{branch}"))
			except Exception as e:
	...
	try:
		with get_session() as db:
			...
			# Extract required attributes while the session is active
			users_data = [
				{
					"id": u.id,
			...
			for u in group:
				...
				with st.expander(f"{u['employee_id']} — {u['name']} ({friendly_role})", expanded=False):
					...
					with col_b1:
						...
						if current_role != "Admin":
							index = child_roles.index(u["role"]) if u["role"] in child_roles else 0
						else:
							index = role_order.index(u["role"]) if u["role"] in role_order else 0
								...
								if ROLE_ORDER[new_role] < ROLE_ORDER[current_role] and current_role != "Admin":
									...
									try:
										with get_session() as tx:
											tu = tx.query(User).get(u["id"])
											if tu and tu.role == "Admin" and new_role != "Admin":
											...
											tu.role = new_role
											if new_password:
												tu.password_hash = pwd.hash(new_password)
											tu.zone_id = None if new_role == "Admin" else z_sel
											tu.region_id = None if new_role in ["Admin","Zone"] else r_sel
											tu.city_id = None if new_role in ["Admin","Zone","Region"] else c_sel
											tu.branch_id = None if new_role in ["Admin","Zone","Region","City"] else b_sel
									...
									try:
										with get_session() as audit_sess:
											# Re-fetch for accurate latest values
											au = audit_sess.query(User).get(u["id"]) 
											if au:
												audit_sess.add(AuditLog(user_id=current_user_id, action="update_user", details=f"{au.employee_id}:{au.name}:{au.role}:{au.zone_id}/{au.region_id}/{au.city_id}/{au.branch_id}"))
									except Exception as e:
										st.warning(f"Audit log not recorded: {str(e)}")
					with col_d2:
							...
							if u["employee_id"] == "C32722":
								...
								try:
									with get_session() as tx:
										...
										if tu_del and tu_del.role == "Admin":
											admins_count = tx.query(User).filter(User.role == "Admin").count()
											if admins_count <= 1:
										...
										if not tu_del:
											st.error("User not found for deletion")
											st.stop()
										tx.delete(tu_del)
