	...
	with st.form("admin_user_form"):
		...
		if sub:
			...
			with get_session() as db:
				...
				# Audit log
				db.add(AuditLog(user_id=current_user_id, action=action, details=f"{emp_id}:{name}:{role}:{user.zone_id}/{user.region_id}/{user.city_id}/{user.branch_id}"))
								...
								if u.employee_id == "C32722":
									...
									if ROLE_ORDER[new_role] < ROLE_ORDER[current_role] and current_role != "Admin":
										...
										with get_session() as tx:
											...
											tx.merge(u)
											# Audit
											tx.add(AuditLog(user_id=current_user_id, action="update_user", details=f"{u.employee_id}:{u.name}:{u.role}:{u.zone_id}/{u.region_id}/{u.city_id}/{u.branch_id}"))
							...
							if st.button("Delete", type="secondary", key=f"del_{u.id}"):
								...
								if u.employee_id == "C32722":
									...
									with get_session() as tx:
										...
										tx.delete(tu_del)
										# Audit
										tx.add(AuditLog(user_id=current_user_id, action="delete_user", details=f"{u.employee_id}:{u.name}:{u.role}:{u.zone_id}/{u.region_id}/{u.city_id}/{u.branch_id}"))
